package org.example.base1.repositorio;
...
import org.example.base1.utiles.ConfigFile;
...
public class ConexionMongoDB {
    ...
    public boolean getPing(MongoDatabase database)
    {
        try {
    ...
    public boolean crearConexion() throws MongoException {
        ...
        if (this.connectionString == null) {
            logger.error("No se ha podido crear la conexión con MongoDB");
            return false;
        }
        try {
    ...
    public void mostrarInformacionCluster() {
        if (this.client == null) {
            logger.warn("Intento de mostrar cluster sin una conexión establecida");
            return;
        }

        logger.info("Información del cluster:");
        logger.info("{}", this.client.getClusterDescription());
    }

    public void mostrarBasesDeDatos() {
        if (this.client == null) {
            logger.warn("No hay conexión establecida para listar bases de datos");
            return;
        }

        logger.info("Iniciando listado de bases de datos...");
    ...
    public void mostrarColecciones(String databaseName) {
        try {
            ...
            logger.info("COLECCIONES DE BD : " + databaseName);
    ...
    public MongoCollection<?> getRedorcCollection(String databaseName, String collectionName, Class entidad) throws Exception {
    ...
    public void cerrarConexion() {
        if (this.client != null) {
            this.client.close();
            logger.info("Se ha cerrado la conexión con MongoDB correctamente");
        }
    }
}